...
def main():
    ...
    l = 10
    X = tensor.zeros((N, N))
    z = Delta0 * DeltaZ * N / wavelength * torch.sqrt(10) * 2
    for i in range(N):
        for j in range(N):
...
def analysisMask():
    ...
    def heatmap(i):
        ...
        maskTHz = mask * e.reshape(-1)
        proj = torch.matmul(dms, maskTHz).cpu().numpy()
    ...
    masksThz = torch.from_numpy(np.asarray(masksThz))
    coherenceHadamard = []
    coherenceThz = []
    for i in range(N*N):
        for j in range(i):
            coherenceHadamard.append(torch.dot(torch.conj(masks[:,i].T).to(device), masks[:,j].to(device)).cpu().numpy())
            coherenceThz.append(torch.dot(torch.conj(masksThz[:,i].T).to(device), masksThz[:,j].to(device)).cpu().numpy())
...
def analysisFOV():
    ...
    def heatmap(i):
        ...
        df = DiscreteFresnel(wavelength, Delta, Delta, Delta, Delta, N, N)
        dms = df.computeGD(z)
        EF = torch.matmul(dms, e.reshape(-1)).reshape((N,N)).cpu().numpy()
